...
**Итог (2026-09-25, живая проверка на DAT):** жест из клика в content-скрипте доходит через
`runtime.sendMessage` до `sidePanel.open` — FAB и `ⓘ details` открывают панель сами; тост остаётся фолбэком.
Грабля: в окне, где уже открыта чужая боковая панель (напр. Claude in Chrome), наша не подключается к вкладке.
