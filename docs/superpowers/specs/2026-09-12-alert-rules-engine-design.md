...
- Груз без ставки («call for rate», rate null) проходит правила без minRate/minRpm/score; в релей
  уходит с rate 0, бот печатает «rate: ask» без $/mi. Раньше такие грузы не доходили никогда (green
  требует netRpm).
